...
集群模式：多台服务器上分别部署ZooKeeper。比如准备 3 台 Linux系统，并分别配置一个 Zookeeper 。
...
伪集群模式：在一台服务器上部署多个ZooKeeper。

说是伪集群，但是因为使用`docker`部署，所以也算是真集群，适合实验环境模拟集群使用 。
...
services:
    ...
    zoo3:
        ...
        environment:
            ZOO_MY_ID: 3
            ZOO_SERVERS: server.1=zoo1:2888:3888 server.2=zoo2:2888:3888 server.3=zoo3:2888
```
